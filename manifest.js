...
export function manifest(value,{allowNative=false}={}){
 ...
 if(value.autoStartWhenFull!==undefined&&(typeof value.autoStartWhenFull!=='boolean'||value.autoStartWhenFull&&value.players.max===null))throw new Error('autoStartWhenFull must be boolean and requires a fixed player maximum.');
 if(value.lobbyActions!==undefined&&(!Array.isArray(value.lobbyActions)||value.lobbyActions.length<1||value.lobbyActions.length>8||new Set(value.lobbyActions).size!==value.lobbyActions.length||value.lobbyActions.some(a=>typeof a!=='string'||!/^[a-z][a-zA-Z0-9]{0,39}$/.test(a)||a.startsWith('host')||a==='playAgain')))throw new Error('Invalid lobbyActions. Declare up to eight player setup actions.');
